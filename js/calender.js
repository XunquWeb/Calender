Calender = function(){
	...
	return {
		...
		getDateList: function() {
			...
			for(x in trans_buf){if(trans_buf[x].getFullYear()==cur_year&&trans_buf[x].getMonth()==cur_month){dateelements[trans_buf[x].getDate()-1].className+=' cal_date_choice';}}
			...
			dateelements.bind('click', function(){
				if($(this).attr('class').indexOf('cal_date_choice')==-1){
					...
					for(x in trans_buf){if(trans_buf[x].getTime()==dt.getTime()){trans_buf.splice(x,1);break;}}
				}
			});
		}
	}
